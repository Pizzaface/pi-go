...
// handleExtensionDialogKey is a spec #5 stub — the extensions UI is
// unavailable in spec #1. Always returns (false, nil, nil) so the caller
// proceeds with the next key handler.
func (m *model) handleExtensionDialogKey(key tea.Key) (bool, tea.Model, tea.Cmd) {
	_ = key
	return false, nil, nil
}

// handleExtensionsPanelKey is a spec #5 stub — see handleExtensionDialogKey.
