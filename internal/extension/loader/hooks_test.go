...
`
	...
	// After validation, verify defaults are applied
	// We need to verify the parsed hooks - re-load and check
	// Since the function doesn't return hooks, we test by checking
	// that no error occurs for hooks with missing tools/timeout
	if md.Name != "ext" {
		t.Fatalf("name = %s; want ext", md.Name)
	}
}
