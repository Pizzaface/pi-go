...
TIMEOUT=120  # 2 minutes (Claude is fast)
...
SIMPLE_PROMPT='Reply with exactly: HELLO_PI_E2E. Nothing else.'
START_TIME=$(date +%s)
set +e
...
if [ "$SIMPLE_EXIT" -ne 0 ]; then
    ...
    if echo "$SIMPLE_OUT" | grep -q "HELLO_PI_E2E"; then
        pass "Test 1 PASSED: Got expected response (${SIMPLE_DURATION}s)"
    else
        error "Test 1 FAILED: Response does not contain HELLO_PI_E2E"
...
JSON_PROMPT='What is 2+2? Reply with just the number.'
set +e
